...
Dato che $ \norm{\vb{a}_i} \leq \frac{\delta}{4}\norm{\vb{a}_k}$  \\  la punta del vettore $\vb{a}_j - \vb{A_j A_i^{-1} a_i}$ cade all'interno di un cerchio di centro la punta di $\vb{a}_j$ e di raggio $r = \frac{\delta}{4}\norm{\vb{a}_k}$
\end{minipage} \hfill 
...
\begin{proof}
 Se $m(\vb{A}) = 0 \Longrightarrow A = id \Longrightarrow \alpha$ è una translazione pura. \\
 Suppongo per assurdo che  $0 < m(\vb{A}) \leq \frac{1}{2} $
\end{proof}
...
\begin{tikzpicture}
  ...
  \draw[blue, thick] (0,0) -- (3,7)node[right] {dgfhsjsg};
  \draw[blue, thick] (0,0) -- (-3,7)node[right]{ahsgfjdfg};
