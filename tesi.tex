...
%
%\begin{proof}
%E' necessario distinguere il caso in cui $n = 1$ dal caso in cui $n > 1$. \\
%\begin{itemize}
...
%Se considero la translazione data da $\omega_j \omega_k \omega_{\nu (j,k)}^{-1}$, questa è caratterizzata da un vettore in $\mathbb{R}^n$ di norma $\leq \frac{3}{2}$.
%\end{itemize}
%
%
%
%Considero la translazione data da 
%\[\omega_j \omega_k \omega_{\nu (j,k)}^{-1}  =  \sum_{i = 1}^{n} m_{ijk} \vb{w}_i \]
%Le comoponenti di translazione $\omega_j$ e $  \omega_k $  hanno modulo unitario, mentre il vettore 
%\end{proof}
%	
